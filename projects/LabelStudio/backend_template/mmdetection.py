...
class MMDetection(LabelStudioMLBase):
    ...
    def predict(self, tasks, **kwargs):
        ...
        print(
            f'>>> model_results.pred_instances: {model_results.pred_instances}'
        )
        for item in model_results.pred_instances:
            ...
            for bbox, corner in zip(bboxes, corners):
                ...
                print('left_top_x:', left_top_x, 'left_top_y', left_top_y)

                # _, min_index = torch.min(corner[:, 1], dim=0)
                # left_top_x, left_top_y = corner[min_index, :]
